...
func InitTail(conf []CollectConf, chanSize int) (err error) {
	...
	if len(conf) == 0 {
		loggo.Error("invalid config for log collect, conf:%v", conf)
		return
	}
...
func CreateNewTask(conf CollectConf) {
	...
	if errTail != nil {
		loggo.Error("collect filename[%s] failed, err:%v", conf.LogPath, errTail)
		return
	}
...
func ReadFromTail(tailObj *TailObj) {
	for true {
		...
		case line, ok := <-tailObj.Tail.Lines:
			if !ok {
				loggo.ErrorFormat("tail file close reopen, filename:%s\n", tailObj.Tail.Filename)
		...
		case <-tailObj.ExitChan:
			loggo.ErrorFormat("tail obj will exited, conf:%v", tailObj.Conf)
			return
...
func SendMsgServer() {
	loggo.Error("send msg server forever running....")
	for {
		msg := GetOneLine()
		err := SendToKafka(msg.Msg, msg.Topic)
		if err != nil {
			loggo.Error("send to kafka failed, err:%v", err)
			time.Sleep(time.Second * 2)
			continue
		}
		loggo.Info("send a message to kafka success")
		time.Sleep(time.Second * 2)
	}
}
